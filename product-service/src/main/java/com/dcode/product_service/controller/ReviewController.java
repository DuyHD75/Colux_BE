...
public class ReviewController {
    ...
    private ResponseEntity<Response> createAReview(@RequestBody @Valid ReviewRequest reviewRequest, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            return ResponseEntity.ok().body(
                    getResponse(request, Map.of("Review", reviewCreated), "Review created successfully!", OK)
            );
    ...
    @GetMapping("/public/userId/{userId}")
    public ResponseEntity<Response> getReviewsByUserId(@PathVariable String userId,
                                                       @RequestParam(defaultValue = "0") int page,
                                                       @RequestParam(defaultValue = "10") int size,
                                                       HttpServletRequest request,
                                                       HttpServletResponse response) {
